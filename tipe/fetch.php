...
if (isset($_POST['search']) && $_POST['search'] == true) {
  ...
  if ($pecah_keywoard[0] == "id_genre") {
    ...
    $sql = $connect->prepare("SELECT  id ,judul, gambar,tahun,status, iu, tipe, level, tanggal_update FROM anime WHERE " . $fcari . " and tipe=:tipe order by tanggal_update desc  LIMIT " . $limit_start . "," . $limit) or die("Gagal mengecek user di database") or die("Gagal mengecek user di database");
    $sql->bindParam(":tipe", $tipe);
    $sql->execute(); // Eksekusi querynya

    $sql2 = $connect->prepare("SELECT  COUNT(*)  AS jumlah_row  FROM anime WHERE   " . $fcari . " and tipe=:tipe") or die("Gagal mengecek user di database") or die("Gagal mengecek user di database");
    $sql2->bindParam(":tipe", $tipe);
    $sql2->execute(); // Eksekusi querynya
    $get_jumlah = $sql2->fetch();
  } else if ($pecah_keywoard[0] == "antidonghua") {
    $sql = $connect->prepare("SELECT  id ,judul, gambar,tahun,status, iu, tipe, level, tanggal_update FROM anime WHERE id_genre NOT LIKE '%donghua%' and tipe=:tipe order by tanggal_update desc  LIMIT " . $limit_start . "," . $limit) or die("Gagal mengecek user di database") or die("Gagal mengecek user di database");
    $sql->bindParam(":tipe", $tipe);
    $sql->execute(); // Eksekusi querynya

    $sql2 = $connect->prepare("SELECT  COUNT(*)  AS jumlah_row  FROM anime WHERE   id_genre NOT LIKE '%donghua%' and tipe=:tipe") or die("Gagal mengecek user di database") or die("Gagal mengecek user di database");
    $sql2->bindParam(":tipe", $tipe);
    $sql2->execute(); // Eksekusi querynya
    $get_jumlah = $sql2->fetch();
    ...
    $param = '%' . $keyword . '%';
    $sql = $connect->prepare("SELECT id ,judul, gambar,tahun,status, iu, tipe, level, tanggal_update  FROM anime  WHERE judul LIKE :judul and tipe=:tipe or alternative_judul LIKE :judulalter and tipe=:tipe  order by tanggal_update desc  LIMIT " . $limit_start . "," . $limit) or die("Gagal mengecek user di database") or die("Gagal mengecek user di database");
    $sql->bindParam(':judul', $param);
    $sql->bindParam(':judulalter', $param);
    $sql->bindParam(":tipe", $tipe);
    $sql->execute(); // Eksekusi querynya

    $sql2 = $connect->prepare("SELECT  COUNT(*)  AS jumlah_row  FROM anime WHERE  judul LIKE :judul and tipe=:tipe") or die("Gagal mengecek user di database") or die("Gagal mengecek user di database");
  ...
  $sql = $connect->prepare("SELECT id ,judul, gambar,tahun,status, iu, tipe, level, tanggal_update FROM anime where tipe=:tipe order by tanggal_update desc LIMIT " . $limit_start . "," . $limit) or die("Gagal mengecek user di database");
  $sql->bindParam(":tipe", $tipe);
  $sql->execute();

  $sql2 = $connect->prepare("SELECT COUNT(*) AS jumlah_row  FROM anime where tipe=:tipe");
  $sql2->bindParam(":tipe", $tipe);
  $sql2->execute();
  $get_jumlah = $sql2->fetch();
}
...
<div style="text-align: center;">
  <ul class="pagination" style="">
    ...
    ?>
      <li><a href="javascript:void(0);" onclick="searchWithPagination(1, false,'<?php echo $tipe; ?>')">First</a></li>
      <li><a href="javascript:void(0);" onclick="searchWithPagination(<?php echo $link_prev; ?>, false,'<?php echo $tipe; ?>')">&laquo;</a></li>
    <?php
    }
    ...
    ?>
      <li<?php echo $link_active; ?>><a href="javascript:void(0);" onclick="searchWithPagination(<?php echo $i; ?>, false,'<?php echo $tipe; ?>')"><?php echo $i; ?></a></li>
      <?php
    }
      ?>
      ...
      ?>
        <li><a href="javascript:void(0);" onclick="searchWithPagination(<?php echo $link_next; ?>, false,'<?php echo $tipe; ?>')">&raquo;</a></li>
        <li><a href="javascript:void(0);" onclick="searchWithPagination(<?php echo $jumlah_page; ?>, false,'<?php echo $tipe; ?>')">Last</a></li>
      <?php
      }
