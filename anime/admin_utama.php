...
.form-control {    height: 25px!important;    padding: 0px 0px!important;    font-size: 11px!important;}
.batas{ width: 100%; margin: auto; display:flex;height:67%;flex-flow:column wrap;align-content:flex-start}
#batas-dalam{background: #777777;padding: 0%; width: 65%;    display: flex;    height: 93%;    flex-flow: column wrap;    align-content: flex-start;}
#batas-dalam > #user_form{ width: 50%;    display: flex;    height: 100%;    flex-flow: column wrap;    align-content: flex-start;}
.form-group{  width: 98%!important;  margin: 0px 5px 0px 4px!important;}
.batas-dalam-2{background: #777777;padding:1%; width: 35%; margin: auto;    height: 100%;}

label {    color: #c1c1c1; font-size: 11px!important;margin-bottom: 1px!important;margin-top: 0px!important;}
.Zebra_DatePicker_Icon, .Zebra_DatePicker_Icon_Inside{margin-left: -21px!important;   margin-top: -22.5px!important;}
.alert{display: none; position: fixed;top: 2%;left: 2%;}

.delete, .delete_streaming{    width: 30%;
    padding: 1px 5px;}
.klikedtr{background: #1e64a9;}

.windowBuka-btn-window-streaming{padding: 1% 2.5% 0.5% 2.5%;color: #ffffff;background: #21b15a;cursor: pointer;}
.windowBuka-btn-window-streaming:hover{    background: #28da6f;}
.windowBuka-btn-window-streaming:active{    background: #1f944d;}
.windowBuka-btn-window-download{padding: 1% 2.5% 0.5% 2.5%;color: #ffffff;background: #2190b1;cursor: pointer;}
.windowBuka-btn-window-download:hover{background: #00adff;}
.windowBuka-btn-window-download:active{background: #11657d;}
.windowBuka-btn-window-download-utama{padding: 1% 2.5% 0.5% 2.5%;color: #ffffff;background: #2190b1;cursor: pointer;}
.windowBuka-btn-window-download-utama:hover{background: #00adff;}
.windowBuka-btn-window-download-utama:active{background: #11657d;}
</style>
 <body>
<div class="batas">
	...
	<div class="batas-dalam-2">
		...
		<form method="post" id="user_form_streaming">
...
<form name="crifrm" method="post" target='_blank' autocomplete="off" ><input type="hidden" name="laporan" value="pdf">
<input type="text" class="form-control" style="    width: 11%;
    position: fixed;
    z-index: 2;
    bottom: 240px;
    background: black;
    color: white;"  placeholder="Cari " name="keyword" id="keyword">
...
$(document).on('click', '.edit', function(){
		...
		$.ajax({
			...
			{	
				...
				$('#streaming_id_judul').val('');$('#streaming_id_judul').val(data.vid);


			$('#id').val(data.vid); 
			$('#judul').val(data.vjudul);$('#alternative_judul').val(data.valternative_judul);
...
if(getidjudul == ""){}else{window.open("admin_utama_download.php?idjudul="+getidjudul, "_blank", "toolbar=yes,scrollbars=yes");}
});

$(document).on('click', '.windowBuka-btn-window-download', function(){ 
var getidjudul = $('#streaming_id_judul').val();
if(getidjudul == ""){}else{window.open("admin/window-download.php?idjudul="+getidjudul, "_blank", "toolbar=yes,scrollbars=yes");}
});

$(document).on('click', '.windowBuka-btn-window-streaming', function(){ 
var getidjudul = $('#streaming_id_judul').val();
if(getidjudul == ""){}else{window.open("admin/window-streaming.php?idjudul="+getidjudul, "_blank", "toolbar=yes,scrollbars=yes");}
});
...
function isi_cbx() {
  ...
  function reset_form_streaming() {
				...
				if($('#id').val() == '')
					{$('#form_action_streaming').css('display','none');}else{$('#form_action_streaming').css('display','block');}
				isi_cbx();
}
...
$(document).on('click', '.edit_streaming', function(){
		...
		$.ajax({
			url:"streaming_action.php",
			method:"POST",
			data:{id:id, action_streaming:action},
			dataType:"json",
			success:function(data)
			{	
				$('#id_streaming').val(id);
...
function searchWithPagination_streaming(page_number, search){
  ...
  type: 'POST', // Tentukan type nya POST atau GET
  data: {keyword: $("#id").val(), page: page_number, search: search}, // Set data yang akan dikirim
  dataType: "json",
  beforeSend: function(e) {
  if(e && e.overrideMimeType) {
  e.overrideMimeType("application/json;charset=UTF-8");
