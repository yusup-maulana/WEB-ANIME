...
$phpstatus = $row["status"];
    ...
    <div class="card_tahun"><span>'.$row["tahun"].'</span></div>';
      ...
      if($phpstatus=="completed"){ echo ' <div class="card_status card_status_completed" style-hover="background:green"><span>'.$row["status"].'</span></div>';}else{ echo ' <div class="card_status card_status_blue"><span>'.$row["status"].'</span></div>';}
...
?>
<style type="text/css">
.fetch-card{ display: flex;flex-wrap: wrap;}
...
135deg
,rgba(42, 245, 152, 1) ,rgba(0, 158, 253, 1) 50%,rgba(42, 245, 152, 1));
}
.card .img1 {
  position: absolute;
   top: 0px;
...
.card .title span {    overflow: hidden;
    display: -webkit-box;
    -webkit-line-clamp: 2;
...
.card .card_status:after{    max-width: 100%;
    height: 20px;
    display: block;
    content: "";
    position: relative;
...
.card_status_blue:after{   background: #2d94e2;}
.card_status_completed:after{ background: #159957;  /* fallback for old browsers */
...
.card .card_status {
    ...
    z-index: 1;
    width: 81px;
}.card .card_status span{    position: relative;
    left: -7px;}
.card .card_tahun {
    position: absolute;
    right: 0px;
