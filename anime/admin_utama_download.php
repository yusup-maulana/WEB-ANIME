...
<style type="text/css">
	.klikedtr{background: #1e64a9;}.alert{display: none; position: fixed;top: 2%;left: 2%;}
</style>
<div style="display:flex;">
		<div style="margin: auto;width: 63%;" id="view_download"> <?php include"download_fetch.php"; ?> </div>
		<form method="post" id="user_form_download" style="width: 47%;">
...
function isi_cbx() {
  ...
  function reset_form_download() {
				...
				if($('#download_id_judul').val() == '')
					{$('#form_action_download').css('display','none');}else{$('#form_action_download').css('display','block');}
				isi_cbx();
}
...
$(document).on('click', '.edit_download', function(){
		...
		$.ajax({
			url:"download_action.php",
			method:"POST",
			data:{id:id, action_download:action},
			dataType:"json",
			success:function(data)
			{	
				$('#id_download').val(id);
...
function searchWithPagination_download(page_number, search){
  ...
  data: {keyword: $("#download_id_judul").val(), page: page_number, search: search}, // Set data yang akan dikirim
  dataType: "json",
